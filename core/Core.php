...
class Core {
    ...
    public function getEVEAPI($fullAPIKey = null) {
        if (!empty($fullAPIKey) || empty(self::$api)) {
            ...
            $cachedAPI = new EVEAPICacheDecorator($loggedAPI);
     ...
     */
    public static function getChachedAPIResponse($requestFunction, $requestArguments) {
        $db = CDatabase::getInstance();
        return $db->getChachedAPIResponse($requestFunction, $requestArguments);
    }
